...
2. 模拟技师执行模拟定位并提交记录单
3. CT 图像入库与转发
4. PvMed 或第三方系统执行自动勾画，必要时由医生进行人工勾画补充或兜底
5. 勾画完成后直接进入计划阶段（不再设置勾画审核步骤）
6. 计划待领与计划设计
7. 计划评估
8. 计划复审（可选）
9. 处方生成/同步
...
4. 跟踪自动勾画状态：
   * 进入 `AutoContouringInProgress`，由 `AutoContourMonitor` 工作项跟踪。
   * 自动勾画成功 → `AutoContouringCompleted`，系统自动推进到 `ManualContouringInProgress`，并创建 Pending 的 `ManualContouring` 工作项交由医生完成或确认。
   * 自动勾画失败 → `AutoContourMonitor` 标记为 Rejected，系统按 S1 兜底策略转入 `ManualContouringInProgress` 并创建 Pending 的 `ManualContouring` 工作项。
5. 医生完成手工勾画（或确认自动结果）后，调用 `complete-manual-contouring` 关闭工作项，状态依次推进 `ManualContouringCompleted → ContoursReady → PlanningPending`，无需独立审核步骤。
...
> 说明：早期版本的 `ContouringInProgress`、`ContoursUnderReview`、`ContoursRejected`、`ContourReworkRequired` 仍保留为枚举值以兼容历史数据，但不再出现在当前活跃工作流中。
* PlanningAssigned
* PlanningInProgress
* PlanReady
* PlanUnderReview
...
* 勾画结果引用存在才能进入 `AutoContouringCompleted` 与 `ContoursReady`，进而自动推进到 PlanningPending。
...
* AutoContourMonitor
* ManualContouring（在进入 `ManualContouringInProgress` 时创建为 Pending，由医生完成）

> 说明：`ContourReview`、`ContourSecondReview`、`ContourRework` 已从活跃工作流中移除，相关审核/返工步骤被取消。
...
* ~~S2_CONTOUR_REVIEW_POLICY~~（已废弃，不再使用勾画审核步骤）
* S3_PLAN_DISPATCH
...
#### S2_CONTOUR_REVIEW_POLICY（已废弃）

当前工作流不再设置勾画审核步骤。`ContoursReady` 之后系统会直接自动推进到 `PlanningPending`，并由 S3 触发计划派发。该插槽保留占位以兼容历史配置，但不再被工作流引擎读取。
